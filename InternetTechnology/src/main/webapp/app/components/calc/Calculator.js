...
'use strict';
myApp.directive('calculator', calculator);
function calculator() {
    return {
        restrict: 'E',
        transclude: true,
        templateUrl: 'components/calc/calculator.html',
        scope: {
        },
...
//            ];
            ...
            scope.saveInMemory = function (operator) {
                if (!scope.isExp) {
                    if (scope.memoryExp) {
                        if (operator) {
            ...
            scope.onClickDel = function () {
                if (scope.displayValue) {
                    var str = scope.displayValue.toString();
            ...
            scope.onClickSum = function () {
                if (scope.displayValue) {
                    ...
                    if (scope.memoryExp && this.isNumeric(scope.memoryExp.slice(-1))) {
                        scope.memoryExp += "+"
                    }
                }
                scope.displayValue = null;
            ...
            scope.addDisplayValue = function (value) {
                if (scope.displayValue) {
                    var str = scope.displayValue.toString() + value;
            ...
            scope.crateDecimal = function () {
                if (scope.displayValue) {
                    var check = scope.displayValue.toString().split(",");
                    if (check.length < 2) {
                        check[0] += ".";
